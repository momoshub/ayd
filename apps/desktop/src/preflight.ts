...
// A GUI app opened from Finder inherits a minimal PATH (no /opt/homebrew/bin), so
// look for the Claude Code CLI in the usual install spots before trusting PATH.
 ...
 * Surfaced in the header so the operator sees a missing prerequisite before a run
